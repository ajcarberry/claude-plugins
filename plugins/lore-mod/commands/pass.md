...
Load `Skill: lore-mod:triage-core`, `Skill: lore-mod:triage-razors`, `Skill: lore-mod:triage-validation`. Read `.lore-mod/config.md` and `<work dir>/PROCESS.md`.
...
4. **Dispatch, one agent per issue.** Stage each issue with `scripts/stage-issue.sh <n>`. Run `lore-mod:issue-validator` for bug reports and anything with a repro; `lore-mod:fact-gatherer` for questions and feature requests. Serialize agents that need the server ports. Each agent gets the issue file, the clone, the binaries, the scope rule, and the report format.
