...
Load `Skill: oss-triage:triage-core`, `Skill: oss-triage:triage-razors`, `Skill: oss-triage:triage-validation`, `Skill: oss-triage:triage-reply`. Read `.oss-triage/config.md` and `<work dir>/PROCESS.md`.
...
5. Fill the matching reply template into `repro/<n>/reply-draft.md`. Dispatch `oss-triage:adversarial-reviewer`; apply its fixes.
